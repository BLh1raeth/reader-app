/**
 * Shared device-local calendar day helpers.
 *
 * Every "day" in Reading Analytics — and every persisted local day key in
 * the database — is a device-local calendar day: the YYYY-MM-DD you see on
 * the iPhone, in its current timezone. This module is the single place that
 * defines that semantic, shared by ReadingSession, Excerpts and Analytics
 * (a feature must never copy these functions).
 *
...
// ---------------------------------------------------------------------------
// Legacy backfill planning (Data Core A.1).
// ---------------------------------------------------------------------------

/**
 * One legacy row that still needs its event-time local day frozen.
 * `id` is the row's primary key (TEXT for sessions, INTEGER for excerpts).
 */
export type LocalDayBackfillRow = {
  id: string | number;
  /** The UTC ISO timestamp the day key is derived from. */
  timestamp: string;
};

export type LocalDayBackfillUpdate = {
  id: string | number;
  dayKey: string;
};

/**
 * Plan the one-time legacy backfill: for each row, interpret its UTC
 * timestamp in the device's CURRENT local timezone and freeze the result
 * as the row's local day key. Rows whose timestamp cannot be parsed are
 * left out (they stay NULL and Analytics treats them as invalid) — the
 * backfill never invents a day.
 *
 * Pure and idempotent by construction: it only produces updates for the
 * rows it is given (the migration SELECTs only NULL-key rows), so running
 * it again over already-filled rows yields zero updates.
 */
export function planLocalDayBackfill(
  rows: ReadonlyArray<LocalDayBackfillRow>,
): LocalDayBackfillUpdate[] {
  const updates: LocalDayBackfillUpdate[] = [];
  for (const row of rows) {
    const dayKey = toLocalDayKey(row.timestamp);
    if (dayKey === null) continue;
    updates.push({ id: row.id, dayKey });
  }
  return updates;
}
