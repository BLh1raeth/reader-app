...
export function reportReaderOpen(bookId: string) {
  ...
  const order: ReaderOpenStage[] = [
    'BOOK_TAP', 'READER_ROUTE_MOUNTED', 'BOOK_DATA_READY', 'EPUB_FILE_READ_START', 'EPUB_PREPARE_START', 'EPUB_PREPARE_END', 'EPUB_FILE_READ_END',
    'DOM_MOUNT_START', 'EPUB_TRANSFER_START', 'DOM_READY', 'EPUB_TRANSFER_END', 'FOLIATE_OPEN_START', 'FOLIATE_IMPORT_START', 'FOLIATE_IMPORT_END',
    'BOOK_BUILD_START', 'BOOK_BUILD_END', 'BOOK_OPEN_START', 'BOOK_OPEN_END', 'FOLIATE_OPEN_END', 'STYLE_APPLY_START', 'STYLE_APPLY_END', 'RESTORE_START', 'VIEW_INIT_START',
    'PAGINATION_START', 'PAGINATION_END', 'VIEW_INIT_END', 'RESTORE_END', 'FIRST_PAGE_RENDERED', 'READER_VISIBLE',
  ];
